...
{"pain":0,"data":0,"fly":0,"incu":0,"veto":false,"reason":{"pain":"","data":"","fly":"","incu":""},"biz":"","actions":["",""]}`;
...
function NicheDiagnoser() {
  ...
  // 1. 로컬 스토리지 데이터 로딩 (비어 있을 시 오늘 발굴한 GO 3대 아이디어 디폴트 주입)
  useEffect(() => {
    try {
        ...
        const defaultSeeds = [
          {
            ...
            biz: "공단 청구의 복잡한 삭감 규칙을 RAG와 프롬프트 검수 세트로 셋업해 빠르게 제공하므로, 요양기관장들의 절대적 호응을 얻는 12점 만점 무주공산입니다.",
            actions: [
          ...
          },
          {
            ...
            biz: "성능지 사진 촬영 한 번으로 주요 광고 사이트용 상세 설명을 허위 기재 없이 10초 만에 추출하는 가볍고 뾰족한 12점짜리 유틸리티입니다.",
            actions: [
          ...
          },
          {
            ...
            biz: "농민의 동영상 수집 편의(카톡 전송)와 스마트스토어 상품 정보를 결합해 숏츠를 완전 무인 자동 렌더링하는 B2B형 12점 만점 독점 가설입니다.",
            actions: [
  ...
  // 5. AI 판정 가동 엔진 (askClaude ➔ Gemini API 이식)
  const handleRunAnalysis = async () => {
    ...
    if (geminiApiKey) {
      try {
        ...
        if (response.ok) {
          ...
          obj.fly = clamp(obj.fly || obj.wheel); // 하위 호환
    ...
    setTimeout(() => {
      const mockResult = {
        ...
        actions: [
          '오늘 수립한 12점 평가 가이드를 공부방 서재에 영구 보존하세요.',
  ...
  // 점수 합산 및 판정 공식
  const total = (o) => (o.pain || 0) + (o.data || 0) + (o.fly || 0) + (o.incu || 0);

  const verdict = (o) => {
    if (o.veto) return ['veto', '⛔ 탈락'];
    const t = total(o);
    if (t >= 10) return ['go', 'GO'];
    if (t >= 7) return ['hold', '보류'];
    return ['drop', '손떼'];
  };
  ...
  return (
    <div className="wrap">
      <h1>🎯 니치 발굴기 v1</h1>
      <div className="sub">아이디어를 넣으면 AI가 4축 점수 · 근거 · 사업가능성 · 다음 액션을 자동 판정 · 대표님 개인 참모</div>
      ...
      <div className="card">
        ...
        <div className="hintline">
          4축: ① 페인 강도 · ② 데이터 접근(5경로) · ③ 플라이휠 · ④ 강자 부재 &nbsp;|&nbsp; 규제 원천봉쇄면 자동 탈락
        </div>
        {statusText && (
          <div 
        ...
        <div id="list">
          {sortedItems.length === 0 ? (
            ...
            sortedItems.map((o, idx) => {
              ...
              const bd = o.veto ? '규제 원천봉쇄로 자동 탈락' : `페인 ${o.pain} · 데이터 ${o.data} · 플라이휠 ${o.fly} · 강자부재 ${o.incu}`;
