...
function NicheDiagnoser() {
  ...
  const handleRunAnalysis = async () => {
    ...
    setTimeout(() => {
      ...
      const isEquip = lowerIdea.includes('임대') || lowerIdea.includes('장비') || lowerIdea.includes('배정');
      const isPremium = lowerIdea.includes('권리금') || lowerIdea.includes('상가');
      const isNims = lowerIdea.includes('마약') || lowerIdea.includes('nims') || lowerIdea.includes('수의사');
      ...
      else if (isEquip) kw = "장기임대";
      else if (isPremium) kw = "상가권리금";
      else if (isNims) kw = "동물마약보고";

      const has15Sig = (isYoyang || isCar || isShorts || isUga || isEquip || isPremium || isNims);

      const mockResult = {
        pain: 3,
        data: has15Sig ? 3 : 2,
        fly: (isUga || isEquip || isPremium || isNims) ? 3 : 3, 
        incu: has15Sig ? 3 : 2,
        niche: 3, 
        veto: false,
        keyword: kw,
        minefield: isUga ? "행정사법 제2조(무자격 서류 대리 작성 대행 처벌), 변호사법 제109조 위반 지뢰밭입니다. AI가 직접 소명서를 대리 인쇄/작성해주지 않고 '셀프 소명 가이드 지식 상품'으로 한정하고 복잡한 건은 제휴 행정사에게 리드로 매칭해야 폭사를 방어합니다." 
          : isEquip ? "장비 배정 신청을 직접 대리 서명하거나 관서에 대출 대행 시 행정사법 위반 저촉 우려가 큽니다. 단순 '자격 조건 1:1 진단 및 서식 템플릿 전달' 수준으로 역할을 명확히 쪼개어 우회해야 합니다."
          : isPremium ? "AI가 독자적인 법률 소장이나 권리금 내용증명을 변형 생성해 주면 변호사법 제109조 위반 지뢰가 터집니다. 2026년 대법원 가이드를 준수하여 단순 조항 1:1 대조 및 표준 내용증명 빈 양식 템플릿 제공 선에서 합법화해야 합니다."
          : isNims ? "동물의약품 유통 전산망(NIMS)을 대행 전송하거나 임의 수정 시 약사법/의료법 저촉 소지가 있습니다. 전산망을 건드리지 않고 '진료 차트와 NIMS 보고 파일 간 오차율 전수 대조' 유틸리티로 면책 안전핀을 심어야 합니다."
          : (isYoyang || isCar || isShorts) ? "실무 기관/딜러 정산 오차로 인한 민사적 손해배상 위험 지뢰밭이 존재합니다. 100% 면책 조항 및 유저 최종 확인 동의 장치를 프론트 UI에 배치해야 안전하게 사업을 영위할 수 있습니다." 
          : "감지된 즉각 폭사 수준의 법적 규제 지뢰밭이 없습니다. (안전 구역)",
        reason: isUga ? {
          ...
          pain: "공공 임대장비(하루 100만~200만 원 상당)를 자격 검토 오판으로 놓쳐 사설 장비를 대여해야 하는 소규모 건설 소장의 비용 낭비.",
          data: "지자체별 공공장비 임대 규정 및 실시간 지원 공고 100% 공개 데이터.",
          fly: "현장 소장들의 장비 신청 데이터와 자격 적격 이력이 누적되어 최적의 임대 성공률 예측 DB 해자 획득.",
          incu: "대형 건설 ERP는 1인 소규모 빌라 소장 전용 공공 장비 신청 핏을 무시하여 무주공산.",
          niche: "소규모 빌라 건축 소장 전용의 초니치 타겟 검색 키워드."
        } : isPremium ? {
          pain: "상가 임대인의 권리금 회수 방해 행위로 평생 모은 수천만~수억 원의 가게 권리금을 잃게 될 자영업 사장님들의 피눈물.",
          data: "상가임대차보호법 고시 규정과 대법원 분쟁 판례 데이터 100% 활용 가능.",
          fly: "지역별/업종별 권리금 소송 시 분쟁 이력과 성공 판례가 서버에 누적되어 대한민국 권리금 분쟁 지도 포털로 진화.",
          incu: "로톡 같은 대형 법률 매칭사는 정밀 요건 진단 툴이 없고, 영세 1인 도구가 비어 있는 무주공산.",
          niche: "권리금 보호 요건을 몰라 당황한 자영업 사장님 전용 틈새 타겟 키워드."
        } : isNims ? {
          pain: "동물 마약류 통합관리시스템(NIMS) 전산 보고 시 오타 하나로 영업 정지 및 면허 취소, 마약법 형사입건 폭탄을 맞는 수의사의 극심한 행정 공포.",
          data: "NIMS 정형 보고 규격집 및 수의사회 마약 보고 프로토콜 가이드라인 100% 공개 데이터.",
          fly: "병원 차트와 마약류 보고 불일치 다빈도 케이스 및 약물 명칭 이상치 패턴 데이터가 누적되어 전수 검수 정확도 최강 고도화.",
          incu: "기존 EMR 차트 업체들은 전송 편의만 제공하며, 오보고 이상 감지 검수 비서 시장은 독점 강자가 없음.",
          niche: "마약류 전산 보고 의무를 진 전국 동물병원 수의사 5,000여 명 타겟 극초니치 검색어."
        ...
        biz: isUga ? "대행 행정 서비스를 AI가 직접 유료 제공 시 행정사법 위반 Veto로 즉시 탈락이지만, '셀프 소명 작성 가이드북 패키지 판매 및 행정사 중개 플랫폼' 형태로 피벗하여 15점 만점 합법적 GO를 만들어낼 수 있습니다." 
          : isEquip ? "공공 임대장비 지원 매칭을 단순 정보 제공 및 템플릿 매핑으로 가이드화하여, 리스크 없이 소장들의 절대적 지지를 받을 수 있는 15점 만점 합법 GO 모델입니다."
          : isPremium ? "2026년 대법원 가이드를 정밀 준수하여 1:1 단순 대조표 및 템플릿 제공으로 선을 그었으므로, 변호사법 시비를 원천 차단한 15점 만점 합법적 노다지 영역입니다."
          : isNims ? "차트 전송 대행이 아닌 단순 수치 전수 검수기로 포지셔닝하여 약사법 위반을 우회하고 매달 수의사들의 충성 구독료를 취할 수 있는 15점 만점 킬러 SaaS입니다."
          : (isYoyang || isCar || isShorts) ? "대표님의 명작 15점 가설로서, RAG를 이용해 1인 메이커 체제로 1주일 이내 초고속 빌딩해 즉시 현금 흐름을 만들어낼 수 있는 최상의 비즈니스입니다." 
          : '1인 창업가의 틈새 시장 판단과 데이터 수집 비효율을 해소하는 가성비 마이크로 SaaS 모델로 성공 확률이 높습니다.',
        actions: isUga ? [
          ...
          "지자체별 공공장비 대여 고시 기준안 및 신청 가이드 데이터베이스 RAG 화",
          "소형 빌라 건설 소장 3명을 만나 공공장비 배정 탈락 사례의 정합성 대조 인터뷰",
          "현장 조건 입력 시 최적의 지자체 공공 크레인/굴착기 배정 매칭 리포트 자동 출력 웹 페이지 구현"
        ] : isPremium ? [
          "상가임대차보호법 권리금 회수 보장 조항 및 과거 대법원 분쟁 판례 RAG 데이터 구축",
          "상가 사장님들이 권리금 계약 시 임대인에게 보내는 표준 내용증명 빈 양식 템플릿 입수",
          "상가 임대차 조건 입력 시 10초 만에 법령상 권리금 보호 적격 여부를 매핑해 주는 간이 진단기 런칭"
        ] : isNims ? [
          "개업 수의사 3명을 만나 마약류 통합관리시스템(NIMS) 전산 보고 시 기재 오류 다빈도 케이스 정밀 스캔",
          "동물병원 EMR 차트 처방량과 NIMS 보고 파일의 불일치 이상치를 비교하는 파서 로직 셋업",
          "처방 XML 파일을 업로드하면 오타를 10초 만에 시각화해 짚어주는 미니 검수 챗봇 POC 빌딩"
        ] : (isYoyang || isCar || isShorts) ? [
